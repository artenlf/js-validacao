...
const mensagensDeErro = {
    ...
    cep: {
        ...
        customError: 'O CEP não foi localizado. Verifique se o número foi digitado corretamente.'
    },
    logradouro: {
...
function validaCPF(input) {
    ...
    if (!checaCPFRepetido(cpfFormatado) || !checaEstruturaCPF(cpfFormatado) || cpfFormatado.length !== 11) {
...
function recuperarCEP(input) {
    ...
    if (!input.validity.patternMismatch && !input.validity.valueMissing) {
        fetch(url, options).then(
            ...
            data => {
                if (data.erro) {
                    input.setCustomValidity('O CEP não foi localizado.');
                    return
                }
